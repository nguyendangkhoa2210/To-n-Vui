...
function updateWelcomeDashboard() {
    ...
    const bestScores = ['lop1','lop2','lop3','lop4','lop5'].map(l => parseInt(localStorage.getItem(`mathuniverse_highscore_${l}`) || '0'));
    ...
    if (encourageEl) {
        ...
        if (viewed === 0) {
            encourageEl.textContent = '👋 Bạn chưa xem bài học nào — hãy chọn 1 khối lớp ở trên để bắt đầu!';
        } else if (viewed < 8) {
            encourageEl.textContent = `🌱 Bạn đã xem ${viewed} bài học — tiếp tục khám phá thêm nhé!`;
        } else if (viewed < totalTopics) {
            encourageEl.textContent = `🔥 Ấn tượng! Bạn đã xem ${viewed}/${totalTopics} bài học — sắp phủ kín cả 5 lớp rồi!`;
        } else {
            encourageEl.textContent = `🏆 Xuất sắc! Bạn đã xem hết ${totalTopics}/${totalTopics} bài học — một cao thủ toán học thực thụ!`;
        }
    }
}
...
// ================================================================
// ========== ⏸️ NHẮC NGHỈ GIẢI LAO (SAU 18 PHÚT) ===============
// ================================================================
// Sau 18 phút học liên tục, hiện overlay dễ thương nhắc đứng dậy
// vươn vai — vừa tốt cho mắt/cột sống, vừa là khoảnh khắc thú vị.
// Timer tự reset khi học sinh dismiss overlay.
...
// ================================================================
// ========== 🎁 PHÒNG TRƯNG BÀY — mở từ nav button ============
// ================================================================
// openGallery() mở Forest Book (bao gồm cả badge tab đã tích hợp).
// Hàm này được gọi từ nút navbar thay thế trực tiếp openForestBook().
