...
const GRADE_COLOR  = { 1: 'orange', 2: 'blue', 3: 'green', 4: 'purple', 5: 'cyan' };
const OBJ_EMOJI = ['🌸','🍎','🍊','⭐','🍪'];
...
defineTopic('l1-hinhphang', 1, '🔷', 'Nhận Diện Hình Phẳng', ['hình vuông', 'hình tròn', 'hình tam giác', 'hình chữ nhật'],
    ...
    () => {
        ...
        const choices = [...kinds].sort(() => Math.random() - 0.5).map(k => k.name);
        return {
            text: 'Đây là hình gì?',
    ...
    (d = 1) => {
        ...
        if (isAdd) {
            ...
            return { text: `Đặt tính rồi tính: ${a} + ${b} = ?`,
                visual: placeValueBlocksSVG(Math.floor(a/10), a%10) + `<div class="mu-obj-note">+ ${b} (que thứ hai bạn tự hình dung để cộng dồn)</div>`,
                answer: a + b, tolerance: 0,
                steps: [`Cộng hàng đơn vị, nhớ nếu vượt quá 9.`, `${a} + ${b} = ${a + b}.`] };
        } else {
            const a = randIntC(20, maxN), b = randIntC(10, a - 5);
            return { text: `Đặt tính rồi tính: ${a} − ${b} = ?`,
                visual: placeValueBlocksSVG(Math.floor(a/10), a%10),
                answer: a - b, tolerance: 0,
...
defineTopic('l2-so1000', 2, '🔢', 'Các Số Đến 1000', ['số tròn trăm', 'so sánh số có 3 chữ số'],
    ...
    (d = 1) => {
        ...
        const cmp = a > b ? 1 : (a < b ? -1 : 0);
        return {
            text: `So sánh ${a} và ${b}. Nhập 1 nếu ${a} > ${b}, -1 nếu ${a} < ${b}, 0 nếu ${a} = ${b}.`,
            ...
            answer: cmp, tolerance: 0,
            steps: [`So từ hàng trăm: ${a} và ${b}.`, `Kết quả: ${a} ${cmp===1?'>':cmp===-1?'<':'='} ${b} → nhập ${cmp}.`],
        };
    });
...
defineTopic('l2-duongthang', 2, '📐', 'Đường Thẳng, Đường Cong, Hình Tứ Giác', ['đường thẳng', 'đường cong', 'tứ giác'],
    ...
    () => {
        ...
        const correct = kinds[randIntC(0, kinds.length - 1)];
        const choices = kinds.map(k => k.name).sort(() => Math.random() - 0.5);
...
defineTopic('l3-bangnhanchia', 3, '✳️', 'Bảng Nhân, Bảng Chia 6–9', ['bảng nhân', 'bảng chia', '6 7 8 9'],
    ...
    (d = 1) => {
        ...
        if (isMul) {
            const x = randIntC(1, 10);
            return { text: `Tính: ${n} × ${x} = ?`, answer: n * x, tolerance: 0, steps: [`${n} × ${x} = ${n*x}.`] };
        } else {
            const x = randIntC(1, 10), prod = n * x;
            return { text: `Tính: ${prod} : ${n} = ?`, answer: x, tolerance: 0, steps: [`${prod} : ${n} = ${x}.`] };
        }
    });
...
defineTopic('l3-nhanchiasocos', 3, '🧾', 'Nhân, Chia Số Có Nhiều Chữ Số', ['nhân', 'chia', 'số có 2-3 chữ số'],
    ...
    (d = 1) => {
        ...
        if (isMul) {
            ...
            return { text: `Đặt tính rồi tính: ${a} × ${b} = ?`, answer: a * b, tolerance: 0,
...
defineTopic('l3-so100000', 3, '🔟', 'Các Số Trong Phạm Vi 100 000', ['số tròn nghìn', 'so sánh số lớn'],
    ...
    (d = 1) => {
        ...
        const cmp = a > b ? 1 : (a < b ? -1 : 0);
        return { text: `So sánh ${a.toLocaleString('vi-VN')} và ${b.toLocaleString('vi-VN')}. Nhập 1 nếu số đầu > số sau, -1 nếu <, 0 nếu =.`,
            answer: cmp, tolerance: 0,
            steps: [`So sánh số chữ số, rồi đến từng hàng từ trái sang.`, `Kết quả: nhập ${cmp}.`] };
    });
...
defineTopic('l4-sotunhien', 4, '🔠', 'Số Tự Nhiên Lớn (Hàng Triệu)', ['hàng triệu', 'giá trị hàng', 'đọc số'],
    ...
    (d = 1) => {
        ...
        return { text: `Trong số ${n.toLocaleString('vi-VN')}, chữ số hàng triệu là bao nhiêu?`, answer: millions, tolerance: 0,
...
defineTopic('l4-4phéptinh', 4, '➗', '4 Phép Tính Với Số Tự Nhiên', ['cộng', 'trừ', 'nhân', 'chia', 'nhiều chữ số'],
    ...
    (d = 1) => {
        ...
        if (op === '+') { const a = randIntC(100, scale), b = randIntC(100, scale); return { text: `Tính: ${a} + ${b} = ?`, answer: a+b, tolerance: 0, steps: [`${a} + ${b} = ${a+b}.`] }; }
        if (op === '-') { const a = randIntC(500, scale), b = randIntC(100, a-50); return { text: `Tính: ${a} − ${b} = ?`, answer: a-b, tolerance: 0, steps: [`${a} − ${b} = ${a-b}.`] }; }
        const a = randIntC(12, Math.round(60*d)+15), b = randIntC(3, 12);
        return { text: `Tính: ${a} × ${b} = ?`, answer: a*b, tolerance: 0, steps: [`${a} × ${b} = ${a*b}.`] };
    });
...
defineTopic('l4-tbc', 4, '🧮', 'Tìm Số Trung Bình Cộng', ['trung bình cộng'],
    ...
    (d = 1) => {
        ...
        const tbc = sum / count;
        return { text: `Tính trung bình cộng của các số: ${nums.join(', ')}.`, answer: parseFloat(tbc.toFixed(2)), tolerance: 0.05,
...
defineTopic('l5-sothapphan', 5, '🔣', 'Đọc, Viết, So Sánh Số Thập Phân', ['số thập phân', 'phần nguyên', 'phần thập phân'],
    ...
    (d = 1) => {
        ...
        const cmp = a > b ? 1 : (a < b ? -1 : 0);
        return { text: `So sánh ${a.toString().replace('.', ',')} và ${b.toString().replace('.', ',')}. Nhập 1 nếu số đầu > số sau, -1 nếu <, 0 nếu =.`,
            answer: cmp, tolerance: 0, steps: [`So phần nguyên trước, rồi đến phần thập phân.`, `Kết quả: nhập ${cmp}.`] };
    });
