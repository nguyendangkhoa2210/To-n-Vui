...
// Tạo 1 SpeechSynthesisUtterance dùng chung giọng tiếng Việt + tốc độ/cao độ
// chuẩn cho trẻ em, để speakQuestion (đọc đề) và speakFeedback (đọc đúng/sai)
// nghe đồng nhất, không lặp code cấu hình giọng ở 2 nơi.
function _makeViUtterance(text, viVoice) {
...
function speakQuestion(text, btnId, highlightEl, choiceBtns) {
    ...
    const makeUtt = (t) => _makeViUtterance(t, viVoice);
...
function stopSpeaking() {
    // Safari trên iPhone/iPad có lỗi: gọi cancel() khi KHÔNG có gì đang đọc
    // (hàng đợi rỗng) làm bộ máy đọc bị "kẹt", các lần speak() sau đó im
    // luôn không phát ra tiếng gì nữa — chỉ cancel() khi thực sự đang phát/chờ phát.
    if (_speechSupported && (window.speechSynthesis.speaking || window.speechSynthesis.pending)) {
        window.speechSynthesis.cancel();
    }
    clearAllTtsHighlights();
...
// Đọc to kết quả đúng/sai cho Lớp 1 & 2, ngay sau khi nộp đáp án — dùng
// chung cho cả 3 nơi có chấm điểm (Luyện Công Thức, Ôn Tập, Nhiệm Vụ Hôm Nay).
// Đúng: chỉ đọc "Đúng rồi!" cho gọn, KHÔNG đọc lại phần giải thích (em đã
// làm đúng rồi, không cần nghe thêm). Sai: đọc "Sai rồi." rồi đọc luôn phần
// giải thích, để các em nghe hiểu ngay vì sao sai mà không phải tự đọc chữ.
// Đọc to "Đúng rồi!" / "Sai rồi." + giải thích (nếu sai) cho Lớp 1 & 2, ngay
// sau khi nộp đáp án — dùng chung cho cả 3 nơi có chấm điểm (Luyện Công
// Thức, Ôn Tập, Nhiệm Vụ Hôm Nay). Đúng: chỉ đọc "Đúng rồi!" cho gọn. Sai:
// đọc "Sai rồi." rồi đọc luôn phần giải thích để các em hiểu ngay vì sao sai.
function speakFeedback(isCorrect, explanationText) {
    if (!_speechSupported) return;
    const viVoice = _findVietnameseVoice();
    if (!viVoice) {
        if (!_viVoiceWarned && typeof showToast === 'function') {
    ...
    // Safari trên iPhone/iPad có lỗi: gọi cancel() khi KHÔNG có gì đang đọc
    // làm bộ máy đọc bị "kẹt", các lần speak() sau đó im re — chỉ cancel()
    // (và chỉ trì hoãn phát câu mới) khi thực sự đang có audio phát/chờ phát.
    ...
    const doSpeak = () => {
        if (isCorrect) {
            window.speechSynthesis.speak(_makeViUtterance('Đúng rồi!', viVoice));
            return;
        }
        window.speechSynthesis.speak(_makeViUtterance('Sai rồi.', viVoice));
        if (explanationText) {
            window.speechSynthesis.speak(_makeViUtterance(explanationText, viVoice));
        }
    };

    if (wasSpeaking) setTimeout(doSpeak, 150);
    else doSpeak();
}
...
// Dùng cho câu dạng so sánh (-1/0/1): hiện dấu thật thay vì bắt gõ số âm, vì
// bàn phím số mặc định trên điện thoại thường không có dấu trừ. Áp dụng cho
// MỌI khối lớp (không chỉ Lớp 1-2) vì đây là lỗi bàn phím, không phải mức độ khó.
const COMPARE_OPTIONS = [
    { symbol: '>', value: 1 },
    { symbol: '<', value: -1 },
    { symbol: '=', value: 0 },
];
function _compareSymbol(v) {
...
// Từ Lớp 3 trở lên, các em vẫn tự gõ đáp số — trừ khi đáp án là số thập phân
// hoặc số lớn (>=1000), vì đây là dạng dễ gõ sai nhất (quên dấu phẩy, gõ nhầm
// chữ số) khiến các em bị tính sai dù hiểu đúng kiến thức. Những trường hợp
// này cũng tự bật trắc nghiệm như Lớp 1 & 2.
function _isDecimalOrLargeAnswer(q) {
    return typeof q.answer === 'number' && (!Number.isInteger(q.answer) || Math.abs(q.answer) >= 1000);
}
// Câu hỏi này có nên dùng nút chọn (thay vì gõ tay) không — gồm cả Lớp 1/2,
// câu dạng so sánh (mọi khối lớp), và câu đáp án thập phân/số lớn (mọi khối lớp).
function _isChoiceQuestion(q) {
    return _isChoiceGrade() || (q && (q.inputType === 'compare' || _isDecimalOrLargeAnswer(q)));
}
// Hiển thị số theo quy ước Việt Nam (dấu phẩy cho phần thập phân). Với số
// nguyên thì không có dấu chấm nên không đổi gì.
function _fmtVN(n) {
...
function _buildQuizChoices(q) {
    ...
    // Làm tròn các đáp án nhiễu về đúng số chữ số thập phân của đáp án gốc để
    // tránh lỗi số thực kiểu "1001.6400000000001".
    const decimalPlacesOf = (n) => { const s = String(n); const i = s.indexOf('.'); return i === -1 ? 0 : s.length - i - 1; };
    const places = decimalPlacesOf(answer);
    ...
    while (distractors.size < 3 && guard < 50) {
        ...
        const candidate = roundTo(answer + delta);
    ...
    while (distractors.size < 3 && guard < 100) {
        guard++;
        const candidate = roundTo(answer + step * (distractors.size % 2 === 0 ? 1 : -1));
        step++;
        if (candidate === answer || (candidate < 0 && answer >= 0)) continue;
        distractors.add(candidate);
    }

    const choices = Array.from(distractors).map(_fmtVN);
    choices.push(_fmtVN(answer));
...
function renderChoiceInput(q) {
    ...
    if (q.inputType === 'compare') {
        if (nativeInput) { nativeInput.style.display = 'none'; nativeInput.value = ''; }
        quizState.selectedChoice = undefined;
        wrap.style.display = 'flex';
        wrap.innerHTML = COMPARE_OPTIONS.map(opt => {
            const ttsText = opt.symbol === '>' ? 'lớn hơn' : opt.symbol === '<' ? 'bé hơn' : 'bằng';
            return `<button type="button" class="mu-choice-btn quiz-choice-btn mu-compare-btn" data-value="${opt.value}" data-tts-text="${ttsText}" onclick="selectQuizChoice(this, '${opt.value}')">${opt.symbol}</button>`;
        }).join('');
        return;
    }
...
function renderQuizQuestion() {
    ...
    document.getElementById('quiz-answer-input').placeholder = q.unit
        ? `Chỉ nhập số (đơn vị "${q.unit}" đã có sẵn)`
        : 'Nhập đáp số của bạn';
    ...
    // Focus: nếu câu này dùng nút chọn (Lớp 1/2, dạng so sánh, hoặc đáp án
    // thập phân/số lớn) thì không focus vào ô input ẩn.
    if (!_isChoiceQuestion(q)) {
...
function submitQuizAnswer() {
    ...
    const q = quizState.questions[quizState.currentIndex];
    const feedback = document.getElementById('quiz-feedback');
    const isChoiceGrade = _isChoiceQuestion(q);
    ...
    if (q.inputType === 'compare') {
        if (quizState.selectedChoice === undefined) {
            feedback.style.display = 'block';
            feedback.className = 'output-box is-error';
            feedback.innerHTML = '⚠️ Hãy chạm chọn một dấu so sánh trước khi xác nhận nhé.';
            startQuizTimer();
            return;
        }
        userAnswer = Number(quizState.selectedChoice);
        userAnswerDisplay = _compareSymbol(userAnswer);
        isCorrect = userAnswer === q.answer;
    } else if (isChoiceGrade) {
        ...
        // Bấm chọn đáp án (không gõ số): quizState.selectedChoice có thể chứa
        // dấu phẩy thập phân (VD "8,67") nên cần đổi lại thành dấu chấm trước
        // khi parseFloat, không thì bị cắt cụt ở dấu phẩy.
        userAnswer = parseFloat(String(quizState.selectedChoice).replace(',', '.'));
    ...
    if (isChoiceGrade) {
        document.querySelectorAll('#quiz-choice-row .quiz-choice-btn').forEach((b) => {
            b.disabled = true;
            // dataset.value có thể chứa dấu phẩy thập phân (VD "8,67") — đổi
            // lại dấu chấm trước khi parseFloat.
            const btnValue = parseFloat(String(b.dataset.value).replace(',', '.'));
    ...
    const answerDisplay = q.inputType === 'compare' ? _compareSymbol(q.answer) : q.answer + (q.unit ? ' ' + q.unit : '');
    ...
    // Lớp 1 & 2 (isChoiceGrade): đọc to "Đúng rồi!" hoặc "Sai rồi." + giải
    // thích — áp dụng cho cả Ôn Tập lẫn Nhiệm Vụ Hôm Nay vì đều đi qua hàm này.
    if (isChoiceGrade) {
        const explanation = (q.steps && q.steps.length) ? q.steps.join('. ') : `Đáp án đúng là ${answerDisplay}.`;
