(function () {
    ...
    // Mỗi khối lớp gắn với 1 bạn thú cưng — phải khớp với GRADE_ICONS trong
    // curriculum.js để đồng nhất hình ảnh xuyên suốt web.
    ...
    #mu-user-dropdown {
        position: absolute; top: calc(100% + 10px); right: 0; z-index: 99998;
        ...
        padding: 14px; min-width: 190px; display: none;
    }
    ...
    @media (max-width: 520px) { #mu-grade-badge { padding: 4px 8px; font-size: 11px; } }
    `;
    ...
    function renderGradeBadge(grade, locked) {
        ...
        badge.innerHTML = gradeLockedByParent
            ? `🔒 ${data.emoji} <span>Lớp ${grade}</span>`
            : `${data.emoji} <span>${GRADE_GATE_DATA.length ? 'Lớp ' + grade : ''}</span>`;
    }
    ...
    async function submitForm(mode) {
        ...
        try {
            // Đăng ký công khai luôn tạo tài khoản PHỤ HUYNH — học sinh không tự
            // đăng ký được nữa (xem handle_register trong api/auth.php); tài khoản
            // học sinh chỉ được tạo bởi phụ huynh trong Bảng Điều Khiển.
            ...
            showError('Không kết nối được máy chủ. Kiểm tra Apache/MySQL trong XAMPP đã bật chưa.');
            setBusy(false);
        }
    }
    ...
    function clearLocalAppData() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (APP_KEY_RE.test(key)) toRemove.push(key);
        }
        toRemove.forEach((k) => localStorage.removeItem(k));
    }

    function onAuthenticated(info) {
        window.MU_ROLE = info.role || 'student';

        // Phụ huynh: LUÔN chuyển thẳng sang trang Bảng Điều Khiển riêng
        // (parent.html) — không có đường nào để phụ huynh vào được giao diện
        // học sinh nữa (trước đây có lối "?tool=1" để in đề/xem thống kê, đã
        // bỏ hẳn — phụ huynh chỉ còn đúng 1 việc: quản lý tài khoản của con).
        if (window.MU_ROLE === 'parent') {
            window.location.href = 'parent.html';
            return;
        }

        // Nếu máy này vừa đăng nhập 1 tài khoản KHÁC với tài khoản đã lưu cục bộ
        // trước đó (đổi tài khoản trên máy dùng chung mà không bấm "Đăng xuất" —
        // ví dụ phiên bị mất do tắt máy đột ngột, xoá cookie...) thì dọn sạch
        // dữ liệu cũ TRƯỚC KHI đồng bộ tài khoản mới về, tránh dữ liệu tài khoản
        // trước còn sót lại đè lên tài khoản đang đăng nhập.
        ...
        if (info.locked_grade) {
            // Phụ huynh đã khoá sẵn khối lớp cho con (đặt trong Bảng Điều Khiển)
            // -> dùng luôn khối lớp đó, KHÔNG hỏi lại và KHÔNG cho tự đổi qua
            // huy hiệu (xem renderGradeBadge). Ghi đè cả giá trị đã lưu cục bộ
            // trên máy để không bị lệch nếu phụ huynh vừa đổi lớp cho con.
    ...
    // Bảng Điều Khiển Phụ Huynh chỉ dành cho tài khoản PHỤ HUYNH — học sinh
    // không thấy nút này. Ngược lại, "Học Công Thức" (danh sách bài học, tìm
    // công thức...) chỉ dành cho HỌC SINH — phụ huynh không cần tự học, nên
    // ẩn đi để bảng điều khiển theo dõi con là trọng tâm.
    //
    // Lưu ý: chỉ set class trên <body> rồi để CSS (!important, xem style.css)
    // lo việc ẩn/hiện — KHÔNG set trực tiếp style.display ở đây, vì switchMode()
    // trong app-core.js cũng ghi đè style.display của #menu-learn/#search-wrap-learn
    // mỗi khi đổi chế độ, dễ gây xung đột thứ tự chạy nếu 2 nơi cùng set inline style.
    ...
    function renderUserBadge(name) {
        ...
        if (!widget) {
            ...
            document.getElementById('mu-user-btn').addEventListener('click', (e) => {
                e.stopPropagation();
                document.getElementById('mu-user-dropdown').classList.toggle('open');
            });
        ...
        // Trang này giờ CHỈ có học sinh đăng nhập vào (phụ huynh luôn được
        // chuyển sang parent.html — xem onAuthenticated), nên huy hiệu người
        // dùng không cần phân biệt vai trò phụ huynh/học sinh nữa.
    ...
    async function checkSession() {
        try {
            ...
            if (json.ok && json.logged_in) {
                ...
                // Chưa đăng nhập -> bắt buộc đăng nhập/đăng ký, không còn chế độ
                // "Học ngay" không cần tài khoản nữa (mọi học sinh phải có tài
                // khoản do phụ huynh tạo trong Bảng Điều Khiển).
                overlay.style.display = 'flex';
                renderForm('login');
            }
        } catch (e) {
            // Không có mạng/chưa bật XAMPP: hiện màn đăng nhập kèm cảnh báo — không
            // còn chế độ khách để vào thay thế nữa, người dùng cần server hoạt động.
            overlay.style.display = 'flex';
            renderForm('login');
            showError('Không kết nối được máy chủ. Kiểm tra Apache/MySQL trong XAMPP đã bật chưa, rồi thử lại.');
        }
    }

    // Script này nạp SAU CÙNG (cuối </body>), DOM đã sẵn sàng nên gọi thẳng,
    // không cần chờ DOMContentLoaded (tránh gọi checkSession() 2 lần).
    checkSession();
})();
