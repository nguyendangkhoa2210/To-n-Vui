// ================================================================
// MathUniverse — usage-guard.js
// ----------------------------------------------------------------
// Đếm số PHÚT học trong ngày cho tài khoản HỌC SINH đã đăng nhập, và tự
// khóa màn hình khi đạt giới hạn phụ huynh đã đặt (xem parent.js + api/usage.php). Chỉ được bật bằng window.MU_USAGE.start() từ auth-ui.js
// (onAuthenticated) khi có đăng nhập thật — mọi học sinh giờ đều đăng nhập
// bằng tài khoản do phụ huynh tạo, không còn chế độ khách nữa.
//
// Cách tính: mỗi 60 giây, nếu tab đang được xem (document.visibilityState
// === 'visible') thì gọi 1 lần POST api/usage.php?action=heartbeat để cộng
// 1 phút. Rời tab / thu nhỏ trình duyệt / khoá máy -> không cộng (nhịp bị
// bỏ qua, không dồn bù khi quay lại — đơn giản và đủ dùng cho mục đích
// giới hạn thời gian học, không cần chính xác tới từng giây).
//
// Nạp SAU app-core.js và auth-ui.js trong index.html.
// ================================================================
(function () {
